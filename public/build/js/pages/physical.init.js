...
document.getElementById("qualificationModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Physical Requirement";
        document.getElementById("qualificationModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Physical Requirement";
        document.getElementById("qualificationModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Physical Requirement";
...
function refreshCallbacks() {
        ...
        btn.onclick = function (e) {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-physical").onclick = function () {                        
                        $.ajax({
                            url: route('physical.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('physical.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                if (data.physical.position_id) {
                    positionVal.setChoiceByValue(data.physical.position_id.toString());
                }

                description.value = data.physical.description;

                if (data.physical.icon) {
                    iconVal.setChoiceByValue(data.physical.icon.toString());
                }

                if (data.physical.color) {
                    colorVal.setChoiceByValue(data.physical.color.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these physical requirements ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these physical requirements will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                ...
                $.ajax({
                    url: route('physical.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
        ...
        Swal.fire({
            title: 'Please select at least one physical requirement',
