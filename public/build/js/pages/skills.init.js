...
// Init list
var skillList = new List("skillList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    skillList.page = perPage;
    skillList.update();
});

isCount = new DOMParser().parseFromString(
    skillList.items.slice(-1)[0]._values.id,
    "text/html"
);
...
document.getElementById("skillModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Skill";
        document.getElementById("skillModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Skill";
        document.getElementById("skillModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Skill";
        document.getElementById("skillModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("skillModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#skillList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});
...
addBtn.addEventListener("click", function (e) {
    ...
    if (form.checkValidity()) {
        var formData = new FormData($('#formSkill')[0]);
...
function refreshCallbacks() {
        ...
        btn.onclick = function (e) {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-skill").onclick = function () {
                        $.ajax({
                            url: route('skill.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success:function(data) {
                                if(data.success === true) {
                                    skillList.remove("id", isdeleteid);
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('skill.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                if (data.skill.position_id) {
                    positionVal.setChoiceByValue(data.skill.position_id.toString());
                }

                var cleanedDescription = data.skill.description.replace(/;;/g, '');

                $("#description .ql-editor").html(cleanedDescription);

                if (data.skill.icon) {
                    iconVal.setChoiceByValue(data.skill.icon.toString());
                }

                if (data.skill.color) {
                    colorVal.setChoiceByValue(data.skill.color.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these skills ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these skills will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    skillList.remove("id", `${ids_array[i]}`);
                }

                $.ajax({
                    url: route('skill.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
