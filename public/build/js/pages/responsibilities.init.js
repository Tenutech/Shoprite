...
// Init list
var responsibilityList = new List("responsibilityList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    responsibilityList.page = perPage;
    responsibilityList.update();
});

isCount = new DOMParser().parseFromString(
    responsibilityList.items.slice(-1)[0]._values.id,
    "text/html"
);
...
document.getElementById("responsibilityModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Responsibility";
        document.getElementById("responsibilityModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Responsibility";
        document.getElementById("responsibilityModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Responsibility";
        document.getElementById("responsibilityModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("responsibilityModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#responsibilityList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});

var table = document.getElementById("responsibilityTable");
// save all tr
...
addBtn.addEventListener("click", function (e) {
    e.preventDefault();
    var form = document.getElementById("formResponsibility");
    if (form.checkValidity()) {
        var formData = new FormData($('#formResponsibility')[0]);
        var description = $("#description .ql-editor").html();
        formData.set('description', description);

        $.ajax({
            url: route('responsibility.store'),
            type: 'POST',
            data: formData,
            async: false,
            processData: false,
...
editBtn.addEventListener("click", function (e) {
    document.getElementById("exampleModalLabel").innerHTML = "Edit Responsibility";
    var editValues = responsibilityList.get({
        id: idField.value,
    });
    var form = document.getElementById("formResponsibility");
    if (form.checkValidity()) {
        ...
        $.ajax({
            url: route('responsibility.update'),
            type: 'POST',
            data: formData,
            async: false,
            processData: false,
...
function refreshCallbacks() {
    Array.from(removeBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            var itemValues = responsibilityList.get({
                id: itemId,
            });
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-responsibility").onclick = function () {
                        $.ajax({
                            url: route('responsibility.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success:function(data) {
                                if(data.success === true) {
                                    responsibilityList.remove("id", isdeleteid);
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('responsibility.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                if (data.responsibility.position_id) {
                    positionVal.setChoiceByValue(data.responsibility.position_id.toString());
                }

                var cleanedDescription = data.responsibility.description.replace(/;;/g, '');

                $("#description .ql-editor").html(cleanedDescription);

                if (data.responsibility.icon) {
                    iconVal.setChoiceByValue(data.responsibility.icon.toString());
                }

                if (data.responsibility.color) {
                    colorVal.setChoiceByValue(data.responsibility.color.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these responsibilities ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these responsibilities will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    responsibilityList.remove("id", `${ids_array[i]}`);
                }

                $.ajax({
                    url: route('responsibility.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
        ...
        Swal.fire({
            title: 'Please select at least one responsibility',
