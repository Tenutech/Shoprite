...
// Init list
var experienceList = new List("experienceList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    experienceList.page = perPage;
    experienceList.update();
});

isCount = new DOMParser().parseFromString(
    experienceList.items.slice(-1)[0]._values.id,
    "text/html"
);
...
document.getElementById("experienceModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Experience";
        document.getElementById("experienceModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Experience";
        document.getElementById("experienceModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Experience";
        document.getElementById("experienceModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("experienceModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#experienceList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});
...
addBtn.addEventListener("click", function (e) {
    e.preventDefault();
    var form = document.getElementById("formExperience");
    if (form.checkValidity()) {
        var formData = new FormData($('#formExperience')[0]);
...
editBtn.addEventListener("click", function (e) {
    document.getElementById("exampleModalLabel").innerHTML = "Edit Experience";
    var editValues = experienceList.get({
        id: idField.value,
    });
    var form = document.getElementById("formExperience");
    if (form.checkValidity()) {
...
function refreshCallbacks() {
    Array.from(removeBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            var itemValues = experienceList.get({
                id: itemId,
            });
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-experience").onclick = function () {                        
                        $.ajax({
                            url: route('experience.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success:function(data) {            
                                if(data.success === true) {
                                    experienceList.remove("id", isdeleteid);
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('experience.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                if (data.experience.position_id) {
                    positionVal.setChoiceByValue(data.experience.position_id.toString());
                }

                description.value = data.experience.description;

                if (data.experience.icon) {
                    iconVal.setChoiceByValue(data.experience.icon.toString());
                }

                if (data.experience.color) {
                    colorVal.setChoiceByValue(data.experience.color.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these experience requirements ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these experience requirements will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    experienceList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('experience.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
        ...
        Swal.fire({
            title: 'Please select at least one experience requirement',
