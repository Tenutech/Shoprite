...
// Craete Reviews For Selected Users
function createReviews(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        ...
        $.ajax({
            url: '/admin/review_cycles',
            type: 'get',
            success:function(data){
                var cycles = $.parseJSON(data);
                ...
                for (var i = 0; i < cycles.length; i++) {
                    ...
                    option.value = cycles[i].cycle_id;
                    option.text = cycles[i].cycle_name;
                    if (cycles[i].active == 1) {
                        value = cycles[i].cycle_id;
                    }
                    select.add(option);
                }
                ...
                Swal.fire({
                    html:   '<div class="mt-3">' + 
                                '<lord-icon src="https://cdn.lordicon.com/jvucoldz.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + 
                                '<div class="mt-4 pt-2 fs-15 mx-5">' + 
                                    '<h4>You are about to create reviews for '+ ids_array.length + ' users ?</h4>' + 
                                        '<p class="text-muted mx-4 mb-0">You are about to generate performance reviews for the selected users. Are you sure?</p>' + 
                                '</div>' +  
                            '</div>' + 
                            '<div class="mt-3">' + 
                                '<div class="form-group">' +
                                    '<label for="cycle-select">Select Review Cycle:</label>' +
                                    select.outerHTML +
                                '</div>' + 
                            '</div>',
                    didOpen: function() {
                        $("#cycle-select").val(value);
                    },
                    showCancelButton: true,
                    confirmButtonClass: 'btn btn-primary w-xs me-2 mt-2',
                    cancelButtonClass: 'btn btn-danger w-xs mt-2',
                    confirmButtonText: "Yes, create reviews!",
                    buttonsStyling: false,
                    showCloseButton: true,
                    showLoaderOnConfirm: true,
                    preConfirm: function() {
                        return new Promise(function() {
                            var reviewCycle = document.getElementById('cycle-select').value;
