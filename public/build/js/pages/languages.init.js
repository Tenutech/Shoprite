...
// Init list
var languageList = new List("languageList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    languageList.page = perPage;
    languageList.update();
});

isCount = new DOMParser().parseFromString(
    languageList.items.slice(-1)[0]._values.id,
    "text/html"
);

var idField = document.getElementById("field-id"),
    languageName = document.getElementById("name"),
...
document.getElementById("languageModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Language";
        document.getElementById("languageModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Language";
        document.getElementById("languageModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Language";
        document.getElementById("languageModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("languageModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#languageList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});
...
addBtn.addEventListener("click", function (e) {
    ...
    if (form.checkValidity()) {
        var formData = new FormData($('#formLanguage')[0]);
...
editBtn.addEventListener("click", function (e) {
    document.getElementById("exampleModalLabel").innerHTML = "Edit Language";
    var editValues = languageList.get({
        id: idField.value,
    });
    var form = document.getElementById("formLanguage");
    if (form.checkValidity()) {
...
function refreshCallbacks() {
    Array.from(removeBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            var itemValues = languageList.get({
                id: itemId,
            });
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-language").onclick = function () {                        
                        $.ajax({
                            url: route('language.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success:function(data) {            
                                if(data.success === true) {
                                    languageList.remove("id", isdeleteid);
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('language.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                languageName.value = data.language.name;

                if (data.language.icon) {
                    iconVal.setChoiceByValue(data.language.icon.toString());
                }

                if (data.language.color) {
                    colorVal.setChoiceByValue(data.language.color.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these languages ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these languages will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    languageList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('language.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
