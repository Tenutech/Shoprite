...
// Init list
var hourList = new List("hourList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    hourList.page = perPage;
    hourList.update();
});

isCount = new DOMParser().parseFromString(
    hourList.items.slice(-1)[0]._values.id,
    "text/html"
);
...
document.getElementById("hourModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Working Hour";
        document.getElementById("hourModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Working Hour";
        document.getElementById("hourModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Working Hour";
        document.getElementById("hourModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("hourModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#hourList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});
...
addBtn.addEventListener("click", function (e) {
    e.preventDefault();
    var form = document.getElementById("formHour");
    if (form.checkValidity()) {
        var formData = new FormData($('#formHour')[0]);

        $.ajax({
            url: route('hour.store'),
            type: 'POST',
            data: formData,
            async: false,
            processData: false,
...
function refreshCallbacks() {
        ...
        btn.onclick = function (e) {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-hour").onclick = function () {                        
                        $.ajax({
                            url: route('hour.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('hour.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                if (data.hour.position_id) {
                    positionVal.setChoiceByValue(data.hour.position_id.toString());
                }

                description.value = data.hour.description;

                if (data.hour.icon) {
                    iconVal.setChoiceByValue(data.hour.icon.toString());
                }

                if (data.hour.color) {
                    colorVal.setChoiceByValue(data.hour.color.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these working hours ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these working hours will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    hourList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('hour.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
        ...
        Swal.fire({
            title: 'Please select at least one working hour',
