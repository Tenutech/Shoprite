...
document.getElementById("custom-dtdps-search").addEventListener("input", function () {
...
function customSearch(query) {
    ...
    userList.filter(function(item) {
        ...
        return (
            ...
            values.brand?.toLowerCase().includes(cleanedQuery)
        );
    });
}
...
function refreshCallbacks() {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-user").onclick = function () {
                        $.ajax({
                            url: route('dtdps.destroy', { id: isdeleteid }),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('dtdps.details', { id: itemId }),
                type: 'get',
                data: {
                    "id": itemId
                },
    ...
    if (typeof ids_array !== 'undefined' && ids_array.length > 0) {
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    url: route('dtdps.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
...
/*
|--------------------------------------------------------------------------
| Export DTDPs Table Report
|--------------------------------------------------------------------------
*/

$(document).ready(function() {
    $('#exportDtdpsTableReport').on('click', function(event) {
        event.preventDefault(); // Prevent default action

        // Reference the export button and save its initial width
        var exportBtn = $('#exportDtdpsTableReport');
        ...
        var searchValue = document.getElementById('custom-dtdps-search').value;

        $.ajax({
            url: route('dtdps.export'),
            method: 'POST',
            data: {
                search: searchValue,
            ...
            success: function(response) {
                ...
                link.download = "dtdps_export.xlsx"; // File name
