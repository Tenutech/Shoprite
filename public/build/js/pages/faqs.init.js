...
// Snow Editor
// Store all Quill instances in a dictionary/object to easily access them later
var quillEditors = {};

// Iterate over each element with class 'snow-editor' and initialize Quill
document.querySelectorAll('.snow-editor').forEach(function (item) {
    var editorId = item.id; // Get the ID of each editor to use as a key

    // Define the Quill editor configuration
    var snowEditorData = {
        theme: 'snow',
        modules: {
            toolbar: [
    ...
    // Initialize Quill editor and store the instance
    quillEditors[editorId] = new Quill(item, snowEditorData);
});
...
// Init list
var faqList = new List("faqList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    faqList.page = perPage;
    faqList.update();
});

isCount = new DOMParser().parseFromString(
    faqList.items.slice(-1)[0]._values.id,
    "text/html"
);

var idField = document.getElementById("field-id"),
    faqName = document.getElementById("faqName"),
...
document.getElementById("faqModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit FAQ";
        document.getElementById("faqModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add FAQ";
        document.getElementById("faqModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List FAQ";
        document.getElementById("faqModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("faqModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#faqList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});
...
addBtn.addEventListener("click", function (e) {
    e.preventDefault();
    var form = document.getElementById("formFAQ");
...
function refreshCallbacks() {
        ...
        btn.onclick = function (e) {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-faq").onclick = function () {                        
                        $.ajax({
                            url: route('faqs.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('faqs.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                faqName.value = data.faq.name;

                // Set the content of the editor with Quill API
                quillEditors['description'].clipboard.dangerouslyPasteHTML(data.faq.description);

                if(data.faq.role_id) {
                    roleVal.setChoiceByValue(data.faq.role_id.toString());
                }

                if(data.faq.type) {
                    typeVal.setChoiceByValue(data.faq.type.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these faq templates ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these templates will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    faqList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('faqs.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
        ...
        Swal.fire({
            title: 'Please select at least one faq',
