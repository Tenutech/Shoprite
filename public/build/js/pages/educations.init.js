...
// Init list
var educationList = new List("educationList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    educationList.page = perPage;
    educationList.update();
});

isCount = new DOMParser().parseFromString(
    educationList.items.slice(-1)[0]._values.id,
    "text/html"
);

var idField = document.getElementById("field-id"),
    educationName = document.getElementById("name"),
...
document.getElementById("educationModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Education";
        document.getElementById("educationModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Education";
        document.getElementById("educationModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Education";
        document.getElementById("educationModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("educationModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#educationList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});
...
addBtn.addEventListener("click", function (e) {
    e.preventDefault();
    var form = document.getElementById("formEducation");
    if (form.checkValidity()) {
        var formData = new FormData($('#formEducation')[0]);
...
editBtn.addEventListener("click", function (e) {
    document.getElementById("exampleModalLabel").innerHTML = "Edit Education";
    var editValues = educationList.get({
        id: idField.value,
    });
    var form = document.getElementById("formEducation");
    if (form.checkValidity()) {
...
function refreshCallbacks() {
    Array.from(removeBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            var itemValues = educationList.get({
                id: itemId,
            });
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-education").onclick = function () {                        
                        $.ajax({
                            url: route('education.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success:function(data) {            
                                if(data.success === true) {
                                    educationList.remove("id", isdeleteid);
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('education.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                educationName.value = data.education.name;

                if (data.education.icon) {
                    iconVal.setChoiceByValue(data.education.icon.toString());
                }

                if (data.education.color) {
                    colorVal.setChoiceByValue(data.education.color.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these educations ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these educations will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    educationList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('education.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
        ...
        Swal.fire({
            title: 'Please select at least one education',
