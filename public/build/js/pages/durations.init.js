...
// Init list
var durationList = new List("durationList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    durationList.page = perPage;
    durationList.update();
});

isCount = new DOMParser().parseFromString(
    durationList.items.slice(-1)[0]._values.id,
    "text/html"
);
...
document.getElementById("durationModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Duration";
        document.getElementById("durationModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Duration";
        document.getElementById("durationModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Duration";
        document.getElementById("durationModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("durationModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#durationList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});

var table = document.getElementById("durationTable");
// save all tr
...
editBtn.addEventListener("click", function (e) {
    document.getElementById("exampleModalLabel").innerHTML = "Edit Duration";
    var editValues = durationList.get({
        id: idField.value,
    });
    var form = document.getElementById("formDuration");
    if (form.checkValidity()) {
...
function refreshCallbacks() {
    Array.from(removeBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            var itemValues = durationList.get({
                id: itemId,
            });
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-duration").onclick = function () {                        
                        $.ajax({
                            url: route('duration.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success:function(data) {            
                                if(data.success === true) {
                                    durationList.remove("id", isdeleteid);
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('duration.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                if (data.duration.color) {
                    colorVal.setChoiceByValue(data.duration.color.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these durations ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these durations will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    durationList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('duration.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
        ...
        Swal.fire({
            title: 'Please select at least one duration',
