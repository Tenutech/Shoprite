...
perPageSelect.addEventListener("change", function() {
    ...
    roleList.page = perPage;
    roleList.update();
});

isCount = new DOMParser().parseFromString(
    roleList.items.slice(-1)[0]._values.id,
    "text/html"
);
...
document.getElementById("roleModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        ...
        document.getElementById("roleModal").querySelector(".modal-footer").style.display = "block";
        ...
        document.getElementById("roleModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("roleModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});
...
addBtn.addEventListener("click", function (e) {
    e.preventDefault();
    var form = document.getElementById("fromRole");
    if (form.checkValidity()) {
        var formData = new FormData($('#fromRole')[0]);
...
function refreshCallbacks() {
        ...
        btn.onclick = function (e) {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-role").onclick = function () {                        
                        $.ajax({
                            url: route('role.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('role.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these roles ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these roles will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    roleList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('role.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
