...
// Init list
var provinceList = new List("provinceList", options).on("updated", function (list) {
...
var idField = document.getElementById("field-id"),
    provinceName = document.getElementById("name"),
...
document.getElementById("provinceModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Province";
        document.getElementById("provinceModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Province";
        document.getElementById("provinceModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Province";
        document.getElementById("provinceModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("provinceModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#provinceList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});

var table = document.getElementById("provinceTable");
// save all tr
...
addBtn.addEventListener("click", function (e) {
    e.preventDefault();
    var form = document.getElementById("formProvince");
    if (form.checkValidity()) {
        var formData = new FormData($('#formProvince')[0]);
...
editBtn.addEventListener("click", function (e) {
    document.getElementById("exampleModalLabel").innerHTML = "Edit Province";
    var editValues = provinceList.get({
        id: idField.value,
    });
    var form = document.getElementById("formProvince");
    if (form.checkValidity()) {
...
function refreshCallbacks() {
    Array.from(removeBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            var itemValues = provinceList.get({
                id: itemId,
            });
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-position").onclick = function () {                        
                        $.ajax({
                            url: route('province.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success:function(data) {            
                                if(data.success === true) {
                                    provinceList.remove("id", isdeleteid);
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('province.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                provinceName.value = data.province.name;
            });
        }
    });
}
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these provinces ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these provinces will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    provinceList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('province.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
