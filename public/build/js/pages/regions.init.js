...
// Init list
var regionList = new List("regionList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function () {
    ...
    regionList.page = perPage;
    regionList.update();
});

isCount = new DOMParser().parseFromString(
    regionList.items.slice(-1)[0]._values.id,
    "text/html"
);

var idField = document.getElementById("field-id"),
    regionName = document.getElementById("name"),
...
document.getElementById("regionModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Region";
        document.getElementById("regionModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Region";
        document.getElementById("regionModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Region";
        document.getElementById("regionModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("regionModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#regionList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});
...
addBtn.addEventListener("click", function (e) {
    e.preventDefault();
    var form = document.getElementById("fromRegion");
    if (form.checkValidity()) {
        var formData = new FormData($('#fromRegion')[0]);
...
function refreshCallbacks() {
        ...
        btn.onclick = function (e) {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-region").onclick = function () {
                        $.ajax({
                            url: route('regions.destroy', { id: isdeleteid }),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success: function (data) {
                                if (data.success === true) {
                                    regionList.remove("id", isdeleteid);
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('regions.details', { id: itemId }),
                type: 'GET',
                data: {
                    "id": itemId
                },
...
function deleteMultiple() {
    ...
    if (typeof ids_array !== 'undefined' && ids_array.length > 0) {
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these regions ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these regions will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    regionList.remove("id", `${ids_array[i]}`);
                }

                $.ajax({
                    url: route('regions.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
...
/*
|--------------------------------------------------------------------------
| Export Regions Table Report
|--------------------------------------------------------------------------
*/

$(document).ready(function() {
    $('#exportRegionsTableReport').on('click', function(event) {
        event.preventDefault(); // Prevent default action

        // Reference the export button and save its initial width
        var exportBtn = $('#exportRegionsTableReport');
        ...
        var searchValue = document.getElementById('custom-regions-search').value;

        $.ajax({
            url: route('regions.export'),
            method: 'POST',
            data: {
                search: searchValue,
            ...
            success: function(response) {
                ...
                link.download = "regions_export.xlsx"; // File name
