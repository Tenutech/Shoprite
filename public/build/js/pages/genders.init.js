...
// Init list
var genderList = new List("genderList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    genderList.page = perPage;
    genderList.update();
});

isCount = new DOMParser().parseFromString(
    genderList.items.slice(-1)[0]._values.id,
    "text/html"
);

var idField = document.getElementById("field-id"),
    genderName = document.getElementById("name"),
...
document.getElementById("genderModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Gender";
        document.getElementById("genderModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Gender";
        document.getElementById("genderModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Gender";
        document.getElementById("genderModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("genderModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#genderList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});

var table = document.getElementById("genderTable");
// save all tr
...
addBtn.addEventListener("click", function (e) {
    e.preventDefault();
    var form = document.getElementById("formGender");
    if (form.checkValidity()) {
        var formData = new FormData($('#formGender')[0]);
...
function refreshCallbacks() {
        ...
        btn.onclick = function (e) {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-gender").onclick = function () {                        
                        $.ajax({
                            url: route('gender.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success:function(data) {            
                                if(data.success === true) {
                                    genderList.remove("id", isdeleteid);
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('gender.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                genderName.value = data.gender.name;

                if (data.gender.icon) {
                    iconVal.setChoiceByValue(data.gender.icon.toString());
                }

                if (data.gender.color) {
                    colorVal.setChoiceByValue(data.gender.color.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these genders ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these genders will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    genderList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('gender.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
        ...
        Swal.fire({
            title: 'Please select at least one gender',
