...
if (regionTotalVacancies > 0) {
    percentageFilled = Math.round((regionTotalVacanciesFilled / regionTotalVacancies) * 100);
}
...
if (regionTotalInterviewsScheduled > 0) {
    percentageCompleted = Math.round((regionTotalInterviewsCompleted / regionTotalInterviewsScheduled) * 100);
}
...
var regionTotalApplicantsAppointed = regionTotalApplicantsAppointed || 0;
var percentageAppointed = 0;

// Check for divide by zero and calculate percentage for appointed applicants
if (regionTotalInterviewsScheduled > 0) {
    percentageAppointed = Math.round((regionTotalApplicantsAppointed / regionTotalInterviewsScheduled) * 100);
}
...
var regionTotalApplicantsRegretted = regionTotalApplicantsRegretted || 0;
var percentageRegretted = 0;

// Check for divide by zero and calculate percentage for regretted applicants
if (regionTotalInterviewsScheduled > 0) {
    percentageRegretted = Math.round((regionTotalApplicantsRegretted / regionTotalInterviewsScheduled) * 100);
}
...
var talentPoolData = regionTalentPoolApplicantsByMonth && Object.keys(regionTalentPoolApplicantsByMonth).length > 0
    ? Object.values(regionTalentPoolApplicantsByMonth) // Extract values if not empty
    : new Array(12).fill(0); // If empty, fill the array with 12 zeros (for each month)

var appointedData = regionApplicantsAppointedByMonth && Object.keys(regionApplicantsAppointedByMonth).length > 0
    ? Object.values(regionApplicantsAppointedByMonth) // Extract values if not empty
    : new Array(12).fill(0); // If empty, fill the array with 12 zeros (for each month)

// Get the months (x-axis categories)
var months = Object.keys(regionApplicantsAppointedByMonth).length > 0 
    ? Object.keys(regionApplicantsAppointedByMonth)  // Use the months from data if available
...
// Extract percentages and labels dynamically from regionTalentPoolApplicantsDemographic
var talentPoolApplicantsDemographicSeries = [];
var talentPoolApplicantsDemographicLabels = [];

regionTalentPoolApplicantsDemographic.forEach(function (item) {
...
// Extract percentages and labels dynamically from regionInterviewedApplicantsDemographic
var interviewedApplicantsDemographicSeries = [];
var interviewedApplicantsDemographicLabels = [];

regionInterviewedApplicantsDemographic.forEach(function (item) {
...
// Extract percentages and labels dynamically from regionAppointedApplicantsDemographic
var appointedApplicantsDemographicSeries = [];
var appointedApplicantsDemographicLabels = [];

regionAppointedApplicantsDemographic.forEach(function (item) {
...
function updateDashboard(data) {
    ...
    $('#totalVacanciesValue').text(data.regionTotalVacancies);

    // Update total vacancies filled
    $('#totalVacanciesFilledValue').text(data.regionTotalVacanciesFilled);

    // Update total interviews scheduled
    $('#totalInterviewsScheduledValue').text(data.regionTotalInterviewsScheduled);

    // Update total interviews completed
    $('#totalInterviewsCompletedValue').text(data.regionTotalInterviewsCompleted);

    // Update total applicants appointed
    $('#totalApplicantsAppointedValue').text(data.regionTotalApplicantsAppointed);

    // Update total applicants regretted
    $('#totalApplicantsRegrettedValue').text(data.regionTotalApplicantsRegretted);

    // Update average time to shortlist
    $('#averageTimeToShortlistValue').text(data.regionAverageTimeToShortlist);

    // Update average time to hire
    $('#averageTimeToHireValue').text(data.regionAverageTimeToHire);

    // Update region adoption rate
    $('#adoptionRateValue').text(data.regionAdoptionRate + '%');

    // Update average distance talent pool applicants
    $('#averageDistanceTalentPoolApplicantsValue').text(data.regionAverageDistanceTalentPoolApplicants);

    // Update average distance applicants appointed
    $('#averageDistanceApplicantsAppointedValue').text(data.regionAverageDistanceApplicantsAppointed);
    ...
    $('#averageAssessmentScoreApplicantsAppointedValue').text(data.regionAverageAssessmentScoreApplicantsAppointed);

    // Update talent pool applicants
    $('#talentPoolApplicantsValue').text(data.regionTalentPoolApplicants);

    // Update appointed applicants
    $('#applicantsAppointedValue').text(data.regionApplicantsAppointed);

    // Update radial charts
    updateRadialChart(totalVacanciesFilledChart, data.regionTotalVacanciesFilled, data.regionTotalVacancies);
    updateRadialChart(totalInterviewsCompletedChart, data.regionTotalInterviewsCompleted, data.regionTotalInterviewsScheduled);
    updateRadialChart(totalApplicantsAppointedChart, data.regionTotalApplicantsAppointed, data.regionTotalInterviewsScheduled);
    updateRadialChart(totalApplicantsRegrettedChart, data.regionTotalApplicantsRegretted, data.regionTotalInterviewsScheduled);

    // Update the "Talent Pool By Month" chart
    updateLineCharts(talentPoolByMonthChart, data.regionTalentPoolApplicantsByMonth, data.regionApplicantsAppointedByMonth);
...
function updateLineCharts(chartInstance, regionTalentPoolApplicantsByMonth, regionpplicantsAppointedByMonth) {
    ...
    // Get the months (x-axis categories)
    var months = Object.keys(regionTalentPoolApplicantsByMonth).length > 0 
        ? Object.keys(regionTalentPoolApplicantsByMonth)  // Use the months from data if available
        : defaultMonths; // Use default months if data is empty
