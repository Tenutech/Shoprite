...
// Init list
var messageList = new List("messageList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    messageList.page = perPage;
    messageList.update();
});

isCount = new DOMParser().parseFromString(
    messageList.items.slice(-1)[0]._values.id,
    "text/html"
);
...
document.getElementById("messageModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Message";
        document.getElementById("messageModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Message";
        document.getElementById("messageModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Message";
        document.getElementById("messageModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("messageModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#messageList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});
...
var categoryVal = new Choices(category, {
    searchEnabled: true,
    shouldSort: false
});
        ...
        $.ajax({
            ...
            success:function(data) {
                if(data.success == true) {
                    ...
                    messageList.sort('category', { order: "desc" });
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: data.message,
...
editBtn.addEventListener("click", function (e) {
    document.getElementById("exampleModalLabel").innerHTML = "Edit Message";
    var editValues = messageList.get({
        id: idField.value,
    });
    var form = document.getElementById("formMessage");
    if (form.checkValidity()) {
...
function refreshCallbacks() {
    Array.from(removeBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            var itemValues = messageList.get({
                id: itemId,
            });
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-message").onclick = function () {                        
                        $.ajax({
                            url: route('chats.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success:function(data) {            
                                if(data.success === true) {
                                    messageList.remove("id", isdeleteid);
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('chats.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                if(data.chat.state_id) {
                    stateVal.setChoiceByValue(data.chat.state_id.toString());
                }

                if(data.chat.category_id) {
                    categoryVal.setChoiceByValue(data.chat.category_id.toString());
                }

                sort.value = data.chat.sort;
            });
        }
    });
}
...
function clearFields() {
    ...
    categoryVal.removeActiveItems();
    categoryVal.setChoiceByValue("");
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these messages ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these messages will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    messageList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('chats.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
