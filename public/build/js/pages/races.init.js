...
// Init list
var raceList = new List("raceList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    raceList.page = perPage;
    raceList.update();
});

isCount = new DOMParser().parseFromString(
    raceList.items.slice(-1)[0]._values.id,
    "text/html"
);
...
document.getElementById("raceModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Race";
        document.getElementById("raceModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Race";
        document.getElementById("raceModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Race";
        document.getElementById("raceModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("raceModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#raceList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});
...
addBtn.addEventListener("click", function (e) {
    e.preventDefault();
    var form = document.getElementById("fromRace");
    if (form.checkValidity()) {
        var formData = new FormData($('#fromRace')[0]);

        $.ajax({
            url: route('race.store'),
            type: 'POST',
            data: formData,
            async: false,
            processData: false,
...
function refreshCallbacks() {
        ...
        btn.onclick = function (e) {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-race").onclick = function () {                        
                        $.ajax({
                            url: route('race.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('race.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                if (data.race.color) {
                    colorVal.setChoiceByValue(data.race.color.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    raceList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('race.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
        ...
        Swal.fire({
            title: 'Please select at least one race',
