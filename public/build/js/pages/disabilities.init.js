...
// Init list
var disabilityList = new List("disabilityList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    disabilityList.page = perPage;
    disabilityList.update();
});

isCount = new DOMParser().parseFromString(
    disabilityList.items.slice(-1)[0]._values.id,
    "text/html"
);

var idField = document.getElementById("field-id"),
    disabilityName = document.getElementById("name"),
...
document.getElementById("disabilityModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Disability";
        document.getElementById("disabilityModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Disability";
        document.getElementById("disabilityModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Disability";
        document.getElementById("disabilityModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("disabilityModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#disabilityList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});

var table = document.getElementById("disabilityTable");
// save all tr
...
addBtn.addEventListener("click", function (e) {
    e.preventDefault();
    var form = document.getElementById("formDisability");
    if (form.checkValidity()) {
        var formData = new FormData($('#formDisability')[0]);

        $.ajax({
            url: route('disability.store'),
            type: 'POST',
            data: formData,
            async: false,
            processData: false,
...
editBtn.addEventListener("click", function (e) {
    document.getElementById("exampleModalLabel").innerHTML = "Edit Disability";
    var editValues = disabilityList.get({
        id: idField.value,
    });
    var form = document.getElementById("formDisability");
    if (form.checkValidity()) {
...
function refreshCallbacks() {
    Array.from(removeBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            var itemValues = disabilityList.get({
                id: itemId,
            });
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-disability").onclick = function () {                        
                        $.ajax({
                            url: route('disability.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success:function(data) {            
                                if(data.success === true) {
                                    disabilityList.remove("id", isdeleteid);
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('disability.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                disabilityName.value = data.disability.name;

                if (data.disability.icon) {
                    iconVal.setChoiceByValue(data.disability.icon.toString());
                }

                if (data.disability.color) {
                    colorVal.setChoiceByValue(data.disability.color.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these disabilities ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these disabilities will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    disabilityList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('disability.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
        ...
        Swal.fire({
            title: 'Please select at least one disability',
