...
function refreshCallbacks() {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-user").onclick = function () {                        
                        $.ajax({
                            url: route('super-admins.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('super-admins.details', {id: itemId}),
                type: 'get',
                data: {
                    "id": itemId
                },
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    url: route('super-admins.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
...
$(document).ready(function() {
    $('#exportSuperAdminsTableReport').on('click', function(event) {
        event.preventDefault(); // Prevent default action

        // Reference the export button and save its initial width
        var exportBtn = $('#exportSuperAdminsTableReport');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                link.download = "super_admins_report.xlsx"; // File name
