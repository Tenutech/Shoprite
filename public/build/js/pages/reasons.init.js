...
// Init list
var reasonList = new List("reasonList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    reasonList.page = perPage;
    reasonList.update();
});

isCount = new DOMParser().parseFromString(
    reasonList.items.slice(-1)[0]._values.id,
    "text/html"
);

var idField = document.getElementById("field-id"),
    reasonName = document.getElementById("name"),
...
document.getElementById("reasonModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Reason";
        document.getElementById("reasonModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Reason";
        document.getElementById("reasonModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Reason";
        document.getElementById("reasonModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("reasonModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#reasonList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});

var table = document.getElementById("reasonTable");
// save all tr
...
addBtn.addEventListener("click", function (e) {
    e.preventDefault();
    var form = document.getElementById("formReason");
    if (form.checkValidity()) {
        var formData = new FormData($('#formReason')[0]);
...
function refreshCallbacks() {
        ...
        btn.onclick = function (e) {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-reason").onclick = function () {                        
                        $.ajax({
                            url: route('reason.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success:function(data) {            
                                if(data.success === true) {
                                    reasonList.remove("id", isdeleteid);
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('reason.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                reasonName.value = data.reason.name;

                if (data.reason.icon) {
                    iconVal.setChoiceByValue(data.reason.icon.toString());
                }

                if (data.reason.color) {
                    colorVal.setChoiceByValue(data.reason.color.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these reasons ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these reasons will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    reasonList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('reason.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
