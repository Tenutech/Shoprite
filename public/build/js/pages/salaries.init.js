...
// Init list
var salaryList = new List("salaryList", options).on("updated", function (list) {
...
perPageSelect.addEventListener("change", function() {
    ...
    salaryList.page = perPage;
    salaryList.update();
});

isCount = new DOMParser().parseFromString(
    salaryList.items.slice(-1)[0]._values.id,
    "text/html"
);
...
document.getElementById("salaryModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("edit-item-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Edit Salary";
        document.getElementById("salaryModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "none";
        document.getElementById("edit-btn").style.display = "block";
    } else if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add Salary";
        document.getElementById("salaryModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("edit-btn").style.display = "none";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Salary";
        document.getElementById("salaryModal").querySelector(".modal-footer").style.display = "none";
    }
});
ischeckboxcheck();

document.getElementById("salaryModal").addEventListener("hidden.bs.modal", function (e) {
    clearFields();
});

document.querySelector("#salaryList").addEventListener("click", function () {
    refreshCallbacks();
    ischeckboxcheck();
});
...
addBtn.addEventListener("click", function (e) {
    e.preventDefault();
    var form = document.getElementById("formSalary");
    if (form.checkValidity()) {
        var formData = new FormData($('#formSalary')[0]);
...
function refreshCallbacks() {
        ...
        btn.onclick = function (e) {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (isdeleteid == itemId) {
                    document.getElementById("delete-salary").onclick = function () {                        
                        $.ajax({
                            url: route('salary.destroy', {id: isdeleteid}),
                            type: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success:function(data) {            
                                if(data.success === true) {
                                    salaryList.remove("id", isdeleteid);
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.onclick = function (e) {
            ...
            $.ajax({
                url: route('salary.details', {id: itemId}),
                type: 'GET',
                data: {
                    "id": itemId
                },
                ...
                if (data.salary.position_id) {
                    positionVal.setChoiceByValue(data.salary.position_id.toString());
                }

                description.value = data.salary.description;

                if (data.salary.icon) {
                    iconVal.setChoiceByValue(data.salary.icon.toString());
                }

                if (data.salary.color) {
                    colorVal.setChoiceByValue(data.salary.color.toString());
                }
            });
        }
    });
...
function deleteMultiple(){
    ...
    if(typeof ids_array !== 'undefined' && ids_array.length > 0){
        Swal.fire({
            html: '<div class="mt-3">' + '<lord-icon src="https://cdn.lordicon.com/gsqxdxog.json" trigger="loop" colors="primary:#f7b84b,secondary:#f06548" style="width:100px;height:100px"></lord-icon>' + '<div class="mt-4 pt-2 fs-15 mx-5">' + '<h4>You are about to delete these salaries ?</h4>' + '<p class="text-muted mx-4 mb-0">Deleting these salaries will remove all of their information from the database.</p>' + '</div>' + '</div>',
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    salaryList.remove("id", `${ids_array[i]}`);
                }
    
                $.ajax({
                    url: route('salary.destroyMultiple'),
                    type: 'post',
                    data: {
                        ids: ids_array
                    },
        ...
        Swal.fire({
            title: 'Please select at least one salary',
